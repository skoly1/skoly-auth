...
var DefaultCryptoAdapter = class {
  randomBytes(size) {
    ...
    const encoded = jose.base64url.encode(new TextEncoder().encode(timestamp));
  ...
  async hash(data, salt) {
    ...
    const token = await new jose.SignJWT({ data: salt + data }).setProtectedHeader({ alg: "HS256" }).sign(secret);
    return token;
  }
