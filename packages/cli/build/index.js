...
var download = new Command("download").description("Install @skoly/auth-core with database adapter").argument("<adapter>", `Database adapter (${Object.keys(ADAPTERS).join(", ")})`).option("-d, --dir <directory>", "Target directory", ".").action(async (adapter, options) => {
  try {
    const targetDir = import_path.default.resolve(options.dir);
    await import_fs.promises.mkdir(targetDir, { recursive: true });
    if (!(adapter in ADAPTERS)) {
      console.log(source_default.red(`Invalid adapter. Available options: ${Object.keys(ADAPTERS).join(", ")}`));
      return;
    }
    const config = {
      adapter,
    ...
    await import_fs.promises.writeFile(
      import_path.default.join(targetDir, "skoly.config.json"),
...
npm install @skoly/auth-core ${ADAPTERS[adapter]}`
    ));
  } catch (error) {
    if (error instanceof Error) {
      console.error(source_default.red("Error during installation:"), error.message);
    } else {
      console.error(source_default.red("Error during installation:"), error);
    }
    process.exit(1);
  }
});
...
// src/index.ts
var program2 = new Command().name("create-skoly-auth").description("Download framework-specific authentication code").version("0.1.0");
program2.addCommand(download);
